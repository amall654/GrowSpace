...
test('imported completed tasks with unknown history remain editable without fabricating a completion time',async()=>{
  const uid='legacy',path=['users',uid,'tasks','old'] as const;
  await env.withSecurityRulesDisabled(async context=>{
    await setDoc(doc(context.firestore() as unknown as Firestore,...path),{...records.tasks,done:true,completedAt:null,...meta});
  });
  const db=userDb(uid);
  await assertSucceeds(updateDoc(doc(db,...path),{title:'Edited old task',revision:2,lastMutationId:'edit'}));
  await assertFails(setDoc(doc(db,'users',uid,'tasks','new'),{...records.tasks,done:true,completedAt:null,...meta}));
  await assertSucceeds(updateDoc(doc(db,...path),{done:false,completedAt:null,revision:3,lastMutationId:'reopen'}));
});
