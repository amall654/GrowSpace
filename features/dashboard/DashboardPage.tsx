  ...
  return <main className="min-h-screen bg-[#fffaf5] text-slate-900">
    <div className="mx-auto flex min-h-screen max-w-7xl">
      ...
      <section className="min-w-0 flex-1 px-5 py-6 sm:px-8 lg:px-10" dir={language === "ar" ? "rtl" : "ltr"}>
        <header className="flex items-center justify-between gap-4"><div><p className="text-sm font-bold text-orange-500">GrowSpace</p><h1 className="mt-1 text-xl font-black sm:text-2xl">{t.hello}، {profile.name}</h1></div><div className="flex items-center gap-2"><Link href="/" className="hidden rounded-full border border-orange-100 bg-white px-4 py-2 text-sm font-bold text-slate-600 sm:block">{t.back}</Link><button onClick={switchLanguage} className="rounded-full border border-orange-200 bg-white px-4 py-2 text-sm font-bold text-orange-600">{t.language}</button></div></header>
        ...
        {tab === "overview" && <Overview t={t} language={language} tasks={orderedTasks} events={events} completedCount={completedCount} pendingCount={pendingTasks.length} weeklyGoal={profile.weeklyGoal} onAdd={() => setShowTaskForm(true)} onToggle={toggleTask} focusMessage={focusMessage} onFocus={() => { setFocusMessage(true); window.setTimeout(() => setFocusMessage(false), 3000); }} dayMap={languageDayMap} />}
        {tab === "tasks" && <TasksPanel t={t} language={language} tasks={orderedTasks} onAdd={() => setShowTaskForm(true)} onToggle={toggleTask} onRemove={removeTask} />}
        {tab === "schedule" && <SchedulePanel t={t} events={events} onAdd={() => setShowEventForm(true)} dayMap={languageDayMap} />}
        {tab === "library" && <LibraryPanel t={t} books={books} onAdd={() => setShowBookForm(true)} onProgress={updateBook} />}
...
function Select({ name, label, options }: { name: string; label: string; options: Array<[string, string]> }) { return <label className="block text-sm font-bold text-slate-700">{label}<select name={name} className="mt-2 w-full rounded-xl border border-orange-100 bg-[#fffaf5] px-4 py-3 outline-none focus:border-orange-400">{options.map(([value, title]) => <option key={value} value={value}>{title}</option>)}</select></label> }
