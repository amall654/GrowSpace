  ...
  return <main className="min-h-screen bg-[#fffaf5] text-slate-900">
    <div className="mx-auto flex min-h-screen max-w-7xl">
      <aside className="hidden w-64 flex-col border-l border-orange-100 bg-white px-5 py-7 lg:flex" dir={language === "ar" ? "rtl" : "ltr"}>
        ...
        <button onClick={() => setShowProfileForm(true)} className="mt-auto flex w-full items-center gap-3 rounded-3xl border border-orange-100 bg-gradient-to-l from-orange-50 to-white p-3 text-right shadow-sm transition hover:-translate-y-0.5 hover:border-orange-200 hover:shadow-md"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-orange-500 text-base font-black text-white shadow-sm">{profile.name.trim().slice(0, 1).toUpperCase()}</span><span className="min-w-0 flex-1"><span className="block text-xs font-bold text-orange-600">{t.profile}</span><span className="mt-0.5 block truncate text-sm font-black text-slate-800">{profile.name}</span><span className="mt-1 block text-xs text-slate-500">{language === "ar" ? "اضبط هدفك الأسبوعي" : "Set your weekly goal"}</span></span><span className="text-lg text-orange-500">‹</span></button>
      </aside>

      <section className="min-w-0 flex-1 px-5 py-6 sm:px-8 lg:px-10" dir={language === "ar" ? "rtl" : "ltr"}>
        <header className="flex items-center justify-between gap-4"><div><p className="text-sm font-bold text-orange-500">GrowSpace</p><h1 className="mt-1 text-xl font-black sm:text-2xl">{t.hello}، {profile.name}</h1></div><div className="flex items-center gap-2">{userId && <button aria-label={t.signOut} title={t.signOut} onClick={signOut} className="hidden rounded-full border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-500 hover:text-orange-600 sm:block">↗</button>}<button onClick={switchLanguage} className="rounded-full border border-orange-200 bg-white px-4 py-2 text-sm font-bold text-orange-600">{t.language}</button></div></header>
...
function TextArea({ name, label, placeholder }: { name: string; label: string; placeholder?: string }) { return <label className="block text-sm font-bold text-slate-700">{label}<textarea name={name} rows={3} placeholder={placeholder} className="mt-2 w-full rounded-xl border border-orange-100 bg-[#fffaf5] px-4 py-3 outline-none placeholder:text-slate-400 focus:border-orange-400" /></label> }
