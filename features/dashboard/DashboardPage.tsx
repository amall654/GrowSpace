  ...
  return <main className="min-h-screen bg-[#fffaf5] text-slate-900">
    <div className="mx-auto flex min-h-screen max-w-7xl">
      ...
      <section className="min-w-0 flex-1 px-5 py-6 sm:px-8 lg:px-10" dir={language === "ar" ? "rtl" : "ltr"}>
        <header className="flex items-center justify-between gap-4"><div><p className="text-sm font-bold text-orange-500">GrowSpace</p><h1 className="mt-1 text-xl font-black sm:text-2xl">{t.hello}، {profile.name}</h1></div><div className="flex items-center gap-2"><Link href="/" className="hidden rounded-full border border-orange-100 bg-white px-4 py-2 text-sm font-bold text-slate-600 sm:block">{t.back}</Link>{demo && <Link href="/login" className="hidden rounded-full bg-orange-500 px-4 py-2 text-sm font-bold text-white hover:bg-orange-600 sm:block">{language === "ar" ? "أنشئ حسابًا" : "Create account"}</Link>}{userId && <button onClick={signOut} className="hidden rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-500 hover:text-orange-600 sm:block">{t.signOut}</button>}<button onClick={switchLanguage} className="rounded-full border border-orange-200 bg-white px-4 py-2 text-sm font-bold text-orange-600">{t.language}</button></div></header>
