  ...
  return <main className="min-h-screen bg-[#fffaf5] text-slate-900">
    <div className="mx-auto flex min-h-screen max-w-7xl">
      ...
      <section className="min-w-0 flex-1 px-5 py-6 sm:px-8 lg:px-10" dir={language === "ar" ? "rtl" : "ltr"}>
        ...
        {tab === "schedule" && <SchedulePanel t={t} events={events} onAdd={() => setShowEventForm(true)} onRemove={removeEvent} dayMap={languageDayMap} />}
        {tab === "library" && <LibraryPanel t={t} books={books} onAdd={() => setShowBookForm(true)} onProgress={updateBook} onRemove={removeBook} />}
        {tab === "courses" && <CoursesPanel t={t} courses={courses} onAdd={() => setShowCourseForm(true)} onRemove={removeCourse} />}
...
function CoursesPanel({ t, courses, onAdd, onRemove }: { t: typeof dashboardCopy[Language]; courses: Course[]; onAdd: () => void; onRemove: (id: string) => void }) { return <section className="mt-8 rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-orange-100"><div className="flex items-center justify-between"><div><h2 className="text-2xl font-black">{t.courses}</h2><p className="mt-1 text-sm text-slate-500">{courses.length} {t.courses}</p></div><button onClick={onAdd} className="rounded-full bg-orange-500 px-4 py-2 text-sm font-bold text-white">+ {t.addCourse}</button></div><div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{courses.map((course) => <article key={course.id} className="rounded-3xl bg-[#fffaf5] p-5"><div className="flex items-start justify-between gap-3"><span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold ${course.color}`}>{course.code || t.courses}</span><button onClick={() => onRemove(course.id)} className="text-xs font-bold text-slate-400 hover:text-red-500">{t.remove}</button></div><h3 className="mt-5 text-lg font-black">{course.name}</h3></article>)}{courses.length === 0 && <p className="py-10 text-slate-500">{t.noCourses}</p>}</div></section> }
