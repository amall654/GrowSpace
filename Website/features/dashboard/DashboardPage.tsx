...
export default function DashboardPage({ demo = false }: { demo?: boolean }) {
  ...
  function updateBookSummary(id: string, note: string) {
    setBooks((current) => current.map((book) => book.id === id ? { ...book, note } : book));
    if (userId && supabase) void supabase.from("books").update({ note }).eq("id", id).eq("user_id", userId);
  }
  ...
  return <main className="min-h-screen bg-[#fffaf5] text-slate-900">
    <div className="mx-auto flex min-h-screen max-w-7xl">
      ...
      <section className="min-w-0 flex-1 px-5 py-6 sm:px-8 lg:px-10" dir={language === "ar" ? "rtl" : "ltr"}>
        ...
        {tab === "overview" && <Overview t={t} language={language} tasks={orderedTasks} events={orderedEvents} completedCount={completedCount} pendingCount={pendingTasks.length} weeklyGoal={profile.weeklyGoal} onAdd={() => setShowTaskForm(true)} onToggle={toggleTask} focusSeconds={focusSeconds} focusRunning={focusRunning} onFocus={() => setFocusRunning((running) => !running)} onResetFocus={() => { setFocusRunning(false); setFocusSeconds(25 * 60); }} dayMap={languageDayMap} />}
        ...
        {tab === "library" && <LibraryPanel t={t} language={language} books={books} onAdd={() => setShowBookForm(true)} onProgress={updateBook} onSaveSummary={updateBookSummary} onRemove={removeBook} />}
    ...
    {showProfileForm && <ProfilePanel t={t} language={language} profile={profile} onClose={() => setShowProfileForm(false)} onSubmit={updateProfile} />}
...
function LibraryPanel({ t, language, books, onAdd, onProgress, onSaveSummary, onRemove }: { t: typeof dashboardCopy[Language]; language: Language; books: Book[]; onAdd: () => void; onProgress: (id: string, value: number) => void; onSaveSummary: (id: string, note: string) => void; onRemove: (id: string) => void }) {
...
function ProfilePanel({ t, language, profile, onClose, onSubmit }: { t: typeof dashboardCopy[Language]; language: Language; profile: Profile; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) { return <div className="fixed inset-0 z-50 bg-slate-900/30" role="dialog" aria-modal="true"><div className={`absolute inset-y-0 w-full max-w-md bg-white p-6 shadow-2xl sm:p-8 ${language === "ar" ? "right-0" : "left-0"}`} dir={language === "ar" ? "rtl" : "ltr"}><div className="flex items-center justify-between"><div><p className="text-sm font-bold text-orange-500">GrowSpace</p><h2 className="mt-1 text-2xl font-black">{t.profile}</h2></div><button onClick={onClose} className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-xl text-slate-500 hover:bg-orange-50 hover:text-orange-600">×</button></div><div className="mt-8 rounded-3xl bg-gradient-to-bl from-orange-500 to-orange-400 p-6 text-white"><div className="flex items-center gap-4"><span className="flex h-16 w-16 items-center justify-center rounded-2xl bg-white/20 text-2xl font-black">{profile.name.trim().slice(0, 1).toUpperCase()}</span><div><p className="text-sm font-bold text-orange-100">{language === "ar" ? "مساحتك الدراسية" : "Your study space"}</p><h3 className="mt-1 text-xl font-black">{profile.name}</h3></div></div><p className="mt-5 text-sm leading-6 text-orange-50">{language === "ar" ? "خصص اسمك وهدفك الأسبوعي، ثم تابع تقدمك من لوحة التحكم." : "Personalize your name and weekly goal, then track your progress from the dashboard."}</p></div><form onSubmit={onSubmit} className="mt-8 space-y-6"><label className="block text-sm font-black text-slate-700">{t.name}<input name="name" defaultValue={profile.name} required className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5 outline-none transition focus:border-orange-400 focus:bg-white focus:ring-4 focus:ring-orange-50" /></label><label className="block text-sm font-black text-slate-700">{t.weeklyGoal}<input name="weeklyGoal" type="number" min="1" max="99" defaultValue={String(profile.weeklyGoal)} required className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5 outline-none transition focus:border-orange-400 focus:bg-white focus:ring-4 focus:ring-orange-50" /><span className="mt-2 block text-xs font-medium text-slate-500">{language === "ar" ? "عدد المهام التي تريد إنجازها خلال هذا الأسبوع." : "How many tasks you want to complete this week."}</span></label><div className="rounded-2xl bg-orange-50 p-4"><p className="text-xs font-bold text-orange-600">{t.weeklyProgress}</p><p className="mt-1 text-lg font-black text-orange-900">{profile.weeklyGoal} {t.tasksUnit}</p></div><div className="flex gap-3 pt-2"><button type="submit" className="flex-1 rounded-full bg-orange-500 px-5 py-3.5 font-black text-white hover:bg-orange-600">{t.save}</button><button type="button" onClick={onClose} className="rounded-full border border-slate-200 px-5 py-3.5 font-bold text-slate-600 hover:bg-slate-50">{t.cancel}</button></div></form></div></div> }
