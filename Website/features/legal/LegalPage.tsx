...
export default function LegalPage({ kind }: { kind: Kind }) {
  ...
  const items = isArabic
    ? privacy ? [["معاينة لوحة الطالب", "تعرض المعاينة أمثلة تعليمية ثابتة فقط، دون جمع بيانات المستخدم أو قراءة تخزينه المحلي أو الحفظ محليًا أو سحابيًا. تختلف عنها مساحة الحساب المسجل."], ["البيانات التي نحفظها", "نحفظ بيانات الحساب التي تدخلها، مثل البريد الإلكتروني، وبيانات الدراسة التي تضيفها: المواد والمهام والمواعيد والكتب."], ["طريقة الحماية", "تُدار المصادقة والبيانات عبر Supabase، ولا يستطيع أي مستخدم الوصول إلى بيانات مستخدم آخر."], ["تحكمك في بياناتك", "يمكنك تعديل بياناتك أو حذفها من لوحة التحكم. لحذف الحساب نهائيًا، تواصل مع مالك المنصة."]]
    ...
    : privacy ? [["Student dashboard preview", "The preview shows fixed educational examples only. It does not collect user data, read browser storage, or save data locally or to the cloud. Signed-in accounts use a separate workspace."], ["Data we store", "We store the account information and study data you choose to add, including courses, tasks, events, and books."], ["How it is protected", "Authentication and data are managed through Supabase. Each user can access only their own data."], ["Your control", "You can edit or delete your data from the dashboard. Contact the platform owner to permanently remove an account."]]
