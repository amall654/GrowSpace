...
export default function ForgotPasswordPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      service.auth.languageCode=isArabic?"ar":"en";
      await sendPasswordResetEmail(service.auth,email,{url:authRedirectUrl("/login/")});
      setStatus({kind:"success",text:isArabic?"إذا كان البريد مسجلًا، أرسلنا إليه رابط استعادة كلمة المرور.":"If this email is registered, we sent a password reset link."});
    } catch(error) {
      if((error as {code?:string}).code==="auth/user-not-found")setStatus({kind:"success",text:isArabic?"إذا كان البريد مسجلًا، أرسلنا إليه رابط استعادة كلمة المرور.":"If this email is registered, we sent a password reset link."});
      else setStatus({kind:"error",text:authErrorMessage(error,isArabic)});
